...
def _default_log_dir() -> Path:
    """Return the platform-correct default log directory.

    Windows: %LOCALAPPDATA%\\RAMMonitor\\logs
        (Windows convention for per-user app data; survives reinstalls)
    macOS:   ~/Library/Logs/RAMMonitor
        (macOS convention)
    Linux:   ~/.ram_monitor/logs
        (XDG-ish; could be ~/.local/share/ram_monitor but kept simple for v2)

    The env var `RAM_MONITOR_LOG_DIR` always overrides this if set.
    """
    if sys.platform == "win32":
        # %LOCALAPPDATA% is the correct Windows convention for per-user app
        # data that doesn't roam between machines. Falls back to ~ if unset
        # (extremely rare — only happens if the user deleted the env var).
        local_appdata = os.environ.get("LOCALAPPDATA")
        if local_appdata:
            return Path(local_appdata) / "RAMMonitor" / "logs"
        return Path.home() / "AppData" / "Local" / "RAMMonitor" / "logs"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Logs" / "RAMMonitor"
    # Linux / other Unix
    return Path.home() / ".ram_monitor" / "logs"
